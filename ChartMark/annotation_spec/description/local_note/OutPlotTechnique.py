...
class OutPlotTechnique(BaseTechnique):
    ...
    marker可同时包含line、text和rect属性
    data必须是ExternalDataNode类型，用于提供外部数据
    """
    def __init__(self, target: DataItemsTargetNode, 
                 line_color: Optional[str] = "red", line_size: Optional[int] = 2, 
        ...
        参数:
            ...
            line_color: 线条颜色，默认为红色，设为None则不添加线条
            line_size: 线条大小，默认为2
        ...
        # 确保至少有一种标记
        if not (line_color or text_field or rect_color):
            raise ValueError("必须至少指定一种标记（线条、文本或矩形）")
    ...
    def validate(self) -> bool:
        ...
        3. marker必须至少包含line、text或rect中的一种
        ...
        # 检查marker是否至少包含line、text或rect中的一种
        if not self.marker or not (self.marker.line or self.marker.text or self.marker.rect):
            return False
    ...
    def has_line(self) -> bool:
        """检查是否有线条标记"""
        return self.marker and self.marker.line is not None
    ...
    def get_line_size(self) -> Optional[int]:
        """获取线条大小，如果没有则返回None"""
        if self.marker and self.marker.line:
            return self.marker.line.size
        return None
    
    def set_line(self, color: str = "red", size: int = 2) -> bool:
        """
        设置线条标记
        ...
        if self.marker:
            self.marker.add_line_marker(color=color, size=size)
            return True
        return False
    ...
    def from_dict(cls, data: Dict) -> 'OutPlotTechnique':
        ...
        # 确保至少有一种标记
        if not (line_color or text_field or rect_color):
            raise ValueError("必须至少有一种标记（line、text或rect）")
    ...
    def _render_leader_line_with_condition(self, original_vegalite_node: Chart, vegalite_filter: dict,  y2_value: int = -8):
        """
        渲染一个带有条件的引导线，从满足条件的数据点指向特定的目标位置。
        
        Args:
            condition_str: 过滤条件，例如 "datum['Premiere week'] >= 1"
            target_x: 引导线终点的x坐标值，例如 "Only You 只有您"
            y2_value: 引导线终点的y坐标值，默认为-8
        """
        # 设置标记类型为线
        mark_type = "line"
        mark_obj = {
          ...
          "strokeWidth": self.get_line_size()
        }
        
        # 获取x和y的编码
        encoding_init_x_y = {
            **original_vegalite_node.get_field_and_type("x"),
            **original_vegalite_node.get_field_and_type("y"),
        }
        ...
        encoding.set_value("x2", "width")
        encoding.set_value("y2", y2_value)
        
        # 创建图层
        line_layer = LayerItem(mark_obj, encoding)
        ...
        # 设置转换属性
        line_layer.set_property(transform=transform.to_dict())
        
        # 添加图层
        original_vegalite_node.add_layer(line_layer) 
        return original_vegalite_node
    ...
    def _base_render(self, original_vegalite_node: Chart):
        ...
        current_vegalite_node = self._render_leader_line_with_condition(current_vegalite_node, vegalite_filter)
