...
class BaseTrend(BaseAnnotationNode):
    """
    趋势分析注释基类，继承自BaseAnnotationNode
    用于处理趋势分析类型的注释，从字典初始化各个组件
    """
    # 技术名称到技术类的映射
    TECHNIQUE_CLASSES: ClassVar[Dict[str, Type[BaseTechnique]]] = {
        "linear_regression": LinearTechnique,
        # 在这里添加其他趋势分析技术类映射
    }
    
    def __init__(self, trend_obj: Dict):
        """
        从字典初始化趋势分析注释
        
        参数:
            trend_obj: 包含趋势分析注释配置的字典
        """
        if not isinstance(trend_obj, dict):
            raise ValueError("trend_obj必须是字典类型")
    ...
    def _create_technique_instance(self, technique_data: Dict) -> BaseTechnique:
        ...
        if not technique_class:
            raise ValueError(f"不支持的趋势分析技术类型: {name}")
    ...
    def get_linear_techniques(self) -> List[LinearTechnique]:
        """
        获取所有线性回归技术
        ...
        返回:
            LinearTechnique实例列表
        """
        return [tech for tech in self.techniques if isinstance(tech, LinearTechnique)]
    
    def add_linear_technique(self, target, line_color="red", line_size=2, 
    ...
    def parse_techniques_to_vegalite(self, original_vegalite_node: Chart, chart_type: ChartType) -> Dict:
        ...
        参数:
            ...
            chart_type: 图表类型，如'bar', 'line', 'scatter'等
